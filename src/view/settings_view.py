...
class SettingsView(customtkinter.CTkFrame):
    def __init__(self, parent):
        ...
        self.lbl_keybind_note = customtkinter.CTkLabel(
            master=self, text=self.note, font=small_font()
        )
        self.lbl_keybind_note.bind(
            "<Configure>",
            lambda e: self.lbl_keybind_note.configure(
                wraplength=self.lbl_keybind_note.winfo_width() - 10
            ),
        )
        ...
        self.rowconfigure(
            self.num_of_widgets + 1, weight=1
        ...
        for i in range(self.num_of_widgets):
            widget_list[i].grid(
                row=i, column=0, columnspan=2, sticky="nsew", padx=20, pady=20
            )
        ...
        self.btn_save.grid(
            row=self.num_of_widgets + 2, column=0, columnspan=2, pady=20, padx=20
        )
    ...
    def __on_press(self, key):
        ...
        self.entry_keybinds.configure(
            text=f"{settings.keybind_to_text(self.current_keys)}"
        )
