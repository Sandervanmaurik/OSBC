...
### When Activated
In `/multi` workflow, Bot-Developer is activated during **Stage 3: DEVELOPMENT** (parallel with Visual-Debugger and Test-Engineer).
...
- **From Senior-Developer**: User preferences, stop conditions, behavior profile selection

### Responsibilities in Multi-Agent Mode
1. Implement bot class following architecture blueprint
2. Apply human-behavior patterns to ALL actions
3. Integrate findings from Code-Explorer (reuse patterns)
4. Coordinate with Visual-Debugger for template paths
5. Validate against Bot-Developer checklist before completing

### Outputs to Other Agents
- **To Test-Engineer**: Implementation for test creation
- **To Code-Reviewer**: Code files for violation scanning
- **To Senior-Developer**: Completion status + checklist results

### Dialogue Format
```
🤖 Bot-Developer: "Implementing [BotName] class..."
├─ State machine: X states implemented
├─ Randomization: truncated_normal_sample for all delays
├─ BehaviorManager: Integrated with [profile]
