"""
Simple test to verify cooking bot stores options in self.options.
...
# Test by importing just the cooking module and checking the code
cooking_file = os.path.join(
    os.path.dirname(__file__), "..", "src", "model", "osrs", "cooking.py"
)
...
# Read the cooking.py file
with open(cooking_file, "r") as f:
    content = f.read()
...
# Test 3: Check that save_options processes fish_type
print("\nTest 3: save_options() processes 'fish_type' option")
if (
    'elif option == "fish_type":' in content
    and "self.fish_type = options[option]" in content
):
    print("PASSED: save_options correctly sets self.fish_type")
else:
    print("FAILED: save_options doesn't properly handle fish_type")
    sys.exit(1)
...
print("=" * 60)
print("\nThe cooking bot code is correctly structured to persist options.")
print("\nHow it works:")
...
print("5. New bot now has correct fish_type and running_time!")
