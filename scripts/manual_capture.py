...
def capture_current_state(
    description: str = "manual_capture",
...
def capture_inventory_states(
    description: str = "inventory_states",
    window_title: str = "RuneLite",
    output_dir: Optional[Path] = None,
) -> Dict[str, Path]:
    """
...
def capture_skill_states(
    description: str = "skill_states",
    window_title: str = "RuneLite",
    output_dir: Optional[Path] = None,
) -> Dict[str, Path]:
    """
    ...
    try:
        ...
        if window.skill_total_level:
            ...
            cv2.rectangle(
                ...
                (rel_x, rel_y),
                (
                    rel_x + window.skill_total_level.width,
                    rel_y + window.skill_total_level.height,
                ),
                (0, 165, 255),
                1,
            )
...
def capture_bank_states(
    description: str = "bank_states",
    window_title: str = "RuneLite",
    output_dir: Optional[Path] = None,
) -> Dict[str, Path]:
    """
    Capture bank interface with slot overlays for testing.

    IMPORTANT: Open the bank interface in-game BEFORE running this command!
    ...
    # Populate bank slots (call this method to generate the bank slot grid)
    try:
        print("   🔍 Detecting bank interface with OCR...")
        window.locate_bank_slots(verbose=True)
        print(f"   ℹ️  Generated {len(window.bank_slots)} bank slot rectangles")
    except Exception as e:
        print(f"❌ Failed to locate bank slots: {e}")
        import traceback

        traceback.print_exc()
        return {}

    # Capture raw game view (where bank interface appears)
    try:
        game_view_screenshot = window.game_view.screenshot()
        raw_path = session_dir / "bank_raw.png"
        cv2.imwrite(str(raw_path), game_view_screenshot)
        captures["bank_raw"] = raw_path
        print(f"   ✅ Raw bank interface captured")

        # Create overlay with slot numbers
        overlay = game_view_screenshot.copy()
        ...
        for i, slot in enumerate(window.bank_slots):
            ...
            # Draw slot number (only show first 81 slots - 9x9 grid)
            if i < 81:
                cv2.putText(
                    overlay,
                    str(i),
        ...
        cv2.imwrite(str(overlay_path), overlay)
        captures["bank_with_slots"] = overlay_path
        print(f"   ✅ Bank with slot overlay captured ({len(window.bank_slots)} slots)")

        # Capture individual bank slots (first 81 only)
        slots_dir = session_dir / "individual_bank_slots"
        slots_dir.mkdir(exist_ok=True)

        for i, slot in enumerate(window.bank_slots[:81]):  # First 9x9 grid
            try:
                slot_screenshot = slot.screenshot()
                slot_path = slots_dir / f"bank_slot_{i:02d}.png"
                cv2.imwrite(str(slot_path), slot_screenshot)
            except Exception as e:
                print(f"   ⚠️  Failed to capture bank slot {i}: {e}")

        print(f"   ✅ Individual bank slots captured to: {slots_dir}")
        captures["individual_bank_slots_dir"] = slots_dir

    except Exception as e:
        print(f"❌ Failed to capture bank: {e}")
        import traceback
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Auto-OSBC Manual Screenshot Capture Tool"
    )
    ...
    parser.add_argument(
        "--inventory", action="store_true", help="Capture inventory with slot overlays"
    )
    parser.add_argument(
        "--skills", action="store_true", help="Capture skills tab with slot overlays"
    )
    parser.add_argument(
        "--bank",
        action="store_true",
        help="Capture bank with slot overlays (OPEN BANK FIRST!)",
    )
    ...
    try:
        if args.inventory:
            captures = capture_inventory_states(
                args.description, args.window, args.output
            )
        elif args.skills:
            captures = capture_skill_states(args.description, args.window, args.output)
        elif args.bank:
            captures = capture_bank_states(args.description, args.window, args.output)
        elif args.regions:
            captures = capture_specific_regions(
                args.regions, args.description, args.window, args.output
            )
        else:
